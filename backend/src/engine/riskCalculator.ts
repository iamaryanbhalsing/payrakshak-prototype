import { AnalysisResult, RiskLevel, VerdictAction, RedFlag, MultilingualAlert } from '../types';
import { parseUpiUri } from './upiParser';
import { analyzeTextPatterns } from './nlpAnalyzer';
import { verifyReceiverVpa } from './receiverVerifier';
import { MULTILINGUAL_TEMPLATES } from '../data/translations';

export function calculateComprehensiveRisk(
  rawInput: string,
  preferredType: 'text' | 'qr' | 'link' | 'vpa' | 'comprehensive' = 'comprehensive'
): AnalysisResult {
  ...
  const nlpRes = analyzeTextPatterns(textToAnalyze);
  ...
  if (targetVpa) {
    const vpaRes = verifyReceiverVpa(targetVpa, claimedName);
  ...
  let rawRisk = Math.max(
    nlpRes.invertedCollectScore,
    nlpRes.remoteAppScore,
    nlpRes.phishingScore,
    nlpRes.urgencyScore * 0.85,
    nlpRes.advanceFeeScore,
    vpaScore,
    intentScore
  );
  ...
  // Verdict Action
  let verdictAction: VerdictAction = 'PROCEED_SAFE';
  ...
  const multilingualAlerts: Record<string, MultilingualAlert> = {};

  for (const [langCode, langData] of Object.entries(MULTILINGUAL_TEMPLATES)) {
