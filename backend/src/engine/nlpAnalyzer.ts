...
import { KNOWN_THREAT_PATTERNS, KNOWN_SUSPICIOUS_DOMAINS } from './threatDb';

export interface NlpAnalysis {
  urgencyScore: number; // 0 to 100
  invertedCollectScore: number; // 0 to 100
  remoteAppScore: number; // 0 to 100
  advanceFeeScore: number; // 0 to 100
  phishingScore: number; // 0 to 100
  extractedUrls: string[];
  detectedPatterns: string[];
  redFlags: RedFlag[];
}

export function analyzeTextPatterns(text: string): NlpAnalysis {
  ...
  for (const threat of KNOWN_THREAT_PATTERNS) {
    ...
    if (!matched) {
      if (threat.id === 'ELECTRICITY_DISCONNECTION_SCAM') {
        ...
        if (hasPower && hasThreat) matched = true;
        ...
        if (hasBank && hasKycSuspension && (hasAction || extractedUrls.length > 0)) matched = true;
        ...
        if (hasReward && hasDeposit) matched = true;
        ...
        if (hasInvertedPrompt || hasArmyPretext) matched = true;
      }
    }
  ...
  for (const url of extractedUrls) {
    ...
    for (const susDomain of KNOWN_SUSPICIOUS_DOMAINS) {
  ...
  if (
    (normalized.includes('enter') || normalized.includes('type') || normalized.includes('input')) &&
