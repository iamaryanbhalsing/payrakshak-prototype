...
exports.reportRouter.post('/generate-1930', (req, res) => {
    try {
        const { incidentType, suspectVpa, suspectName, suspectPhone, suspectUrl, amountAttempted, rawInput, evidenceNote, detectedRedFlags } = req.body;
        ...
        if (suspectVpa)
            narrative += `• Suspect UPI ID / VPA: ${suspectVpa}\n`;
        if (suspectName)
            narrative += `• Suspect Display / Claimed Name: ${suspectName}\n`;
        if (suspectPhone)
            narrative += `• Suspect Contact Number: ${suspectPhone}\n`;
        if (suspectUrl)
            narrative += `• Malicious Phishing URL: ${suspectUrl}\n`;
        if (amountAttempted)
            narrative += `• Amount Involved / Attempted: ₹${amountAttempted}\n\n`;
        ...
        if (detectedRedFlags && Array.isArray(detectedRedFlags)) {
            ...
            detectedRedFlags.forEach((f, idx) => {
